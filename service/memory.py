...
from config import HOST_MAP, ZABBIX_AUTH, ZABBIX_URL, ITEMIDS_MEMORY_MAP

async def per_mem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print("[LOG] Menjalankan service: mem_command")
    if len(context.args) == 0:
        await update.message.reply_text("Gunakan format: /mem <kode>")
        return
    ...
    try:
        ...
        if "result" in data and len(data["result"]) > 0:
            ...
            await update.message.reply_text(f"Memory Utilization VM {kode}: {value}%")
        else:
...
async def realtime_memory_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print("[LOG] Menjalankan service: realtime_memory_command")
    await update.message.reply_text("⏳ Memory Utilization Summary sedang diproses, mohon ditunggu...")

    # ambil semua hostids dari HOST_MAP
    ...
    try:
        ...
        if "result" in data and len(data["result"]) > 0:
            ...
            for kode in ITEMIDS_MEMORY_MAP.keys():
                iid = ITEMIDS_MEMORY_MAP[kode]
    ...
    summary_text = "📊 Memory Utilization Summary:\n" + "\n".join(results)
...
async def specific_memory_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print("[LOG] Menjalankan service: specific_memory_command")
    if len(context.args) == 0:
    ...
    all_itemids = list(ITEMIDS_MEMORY_MAP.values())
    ...
    results = []
    for kode, iid in ITEMIDS_MEMORY_MAP.items():
        if iid in latest:
    ...
    summary_text = f"📊 Memory Utilization Summary {jam:02d}:{menit:02d} WIB:\n" + "\n".join(results)
    await update.message.reply_text(summary_text)
